...
<head>
	<meta charset="utf-8">
	<title>Deen Car Rental || Admin Login</title>
...
{
$email=$_POST['inputemail'];// Get the email entered by the user
$password=md5($_POST['pass']);// Get the password entered by the user and hash it using MD5 (assuming passwords are stored as MD5 hashes in the database)

$sql ="SELECT Email,Password,Role_id FROM user WHERE Email=:email and Password=:password";
...
if($query->rowCount() > 0)
{
	if($results[0]->Role_id == "1"){// Check if the user is an admin (assuming admin role ID is "1")
