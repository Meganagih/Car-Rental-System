...
$sql="update car set CarName=:vehicletitle,car_nameplate=:namepale,Brand_id=:brand,CarOverview=:vehicleoverview,PricePerDay=:priceperday,Price_Pe_KM=:priceperkm,FuelType=:fueltype,ModelYear=:modelyear,SeatingCapacity=:seatingcapacity,AirConditioner=:airconditioner,PowerDoorLocks=:powerdoorlocks,AntiLockBrakingSystem=:antilockbrakingsys,BrakeAssist=:brakeassist,PowerSteering=:powersteering,DriverAirbag=:driverairbag,PassengerAirbag=:passengerairbag,PowerWindows=:powerwindow,CDPlayer=:cdplayer,CentralLocking=:centrallocking,CrashSensor=:crashcensor,LeatherSeats=:leatherseats where Car_id=:id ";
$query = $dbh->prepare($sql);
$query->bindParam(':vehicletitle',$vehicletitle,PDO::PARAM_STR);
$query->bindParam(':namepale',$namepale,PDO::PARAM_STR);
...
<head>
	...
	<title>Deen Car Rental Portal | Admin Edit Vehicle Info</title>
...
color: white;
font-size: 40px;
font-family: initial;">Edit Vehicle</h2>
...
<?php 
$id=intval($_GET['id']);
$sql ="SELECT car.*,brands.BrandName,brands.Brand_id as bid from car join brands on brands.Brand_id=car.Brand_id where car.Car_id=:id";
$query = $dbh -> prepare($sql);
$query-> bindParam(':id', $id, PDO::PARAM_STR);
$query->execute();
...
<div class="col-sm-4">
<input type="text" name="vehicletitle" class="form-control" value="<?php echo htmlentities($result->CarName);?>" required>
</div>
<label class="col-sm-2 control-label">NamePlate<span style="color:red">*</span></label>
<div class="col-sm-4">
<input type="text" name="namepale" class="form-control" value="<?php echo htmlentities($result->car_nameplate);?>" required>
</div>
</div>
<div class="form-group">
...
<option value="<?php echo htmlentities($result->Brand_id);?>"><?php echo htmlentities($bdname=$result->BrandName); ?> </option>
<?php $ret="select Brand_id,BrandName from brands";
...
?>
<option value="<?php echo htmlentities($results->Brand_id);?>"><?php echo htmlentities($results->BrandName);?></option>
<?php }}} ?>
...
<div class="col-sm-10">
<textarea class="form-control" name="vehicalorcview" rows="3" required><?php echo htmlentities($result->CarOverview);?></textarea>
</div>
</div>

<div class="form-group">
<label class="col-sm-2 control-label">Price Per Day(in LKR)<span style="color:red">*</span></label>
<div class="col-sm-4">
<input type="text" name="priceperday" class="form-control" value="<?php echo htmlentities($result->PricePerDay);?>" required>
</div>
<label class="col-sm-2 control-label">Price Per KM(in LKR)<span style="color:red">*</span></label>
<div class="col-sm-4">
<input type="text" name="priceperkm" class="form-control" value="<?php echo htmlentities($result->Price_Pe_KM);?>" required>
</div>
</div>
<div class="form-group">
...
<div class="form-group">
<div class="col-sm-4">
Image 1 <img src="img/vehicleimages/<?php echo htmlentities($result->Pic1);?>" width="300" height="200" style="border:solid 1px #000">
<a href="changeimage1.php?imgid=<?php echo htmlentities($result->Car_id)?>">Change Image 1</a>
</div>
<div class="col-sm-4">
Image 2<img src="img/vehicleimages/<?php echo htmlentities($result->Pic2);?>" width="300" height="200" style="border:solid 1px #000">
<a href="changeimage2.php?imgid=<?php echo htmlentities($result->Car_id)?>">Change Image 2</a>
</div>
<div class="col-sm-4">
Image 3<img src="img/vehicleimages/<?php echo htmlentities($result->Pic3);?>" width="300" height="200" style="border:solid 1px #000">
...
<div class="form-group">
<div class="col-sm-4">
Image 4<img src="img/vehicleimages/<?php echo htmlentities($result->Pic4);?>" width="300" height="200" style="border:solid 1px #000">
...
<?php }} ?>
											...
											<div class="form-group">
												<div class="col-sm-8 col-sm-offset-2" >
													...
													<center><button class="btn btn-primary submit" name="submit" type="submit" style="margin-top:4%">Update</button></center>
												</div>
											</div>
